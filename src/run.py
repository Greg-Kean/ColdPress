...
def keyboardInterruptHandler(signal, frame):
    global sigint_count
    sigint_count += 1
    eprint("-------KeyboardInterrupt (sig {}) count {}!--------".format(signal,
                                                                        sigint_count), color=C_RED)
    eprint("there are %d threads active:" %
           threading.activeCount(), color=C_WARNING)
...
class cmdThread(threading.Thread):
    ...
    def run(self):
        eprint("started cmdThread %i: %s" %
               (self.native_id, self.cmd), color=C_UNDERLINE)
        start = time.time()
        # subprocess is thread safe! yes! awesome!
        self.output = subprocess.run(self.cmd.split(
        ), stderr=sys.stderr, stdout=subprocess.PIPE).stdout.decode('utf-8')

        self.done = True

        eprint("[%f seconds] cmdThread %i finished command %s, output %s bytes" % (time.time(
        ) - start, self.native_id, self.cmd.split()[0], len(self.output)), color=C_UNDERLINE+C_YELLOW)

    def get_output(self):
        try:
            return self.output
        except AttributeError as e:
            eprint("cmdThread %i:%s has no output" %
                   (self.native_id, self.cmd), e, color=C_WARNING)
...
class funcThread(threading.Thread):
    ...
    def run(self):
        start = time.time()
        eprint("started funcThread %i:" % self.native_id,
               self.funcs, color=C_UNDERLINE+C_CYAN)
        for f in self.funcs:
            func = f[0]
        ...
        eprint("[%f] funcThread %i finished:" % (time.time() - start,
                                                 self.native_id), self.funcs, color=C_UNDERLINE+C_CYAN)
...
class PEInfo:
    ...
    def __init__(self, path, modules_enabled=set(MODULES_ALL)):
        ...
        if 'ghidra' in modules_enabled:
            ghidra_thread = self.start_ghidra(
                os.path.join(OUTPUT_DIR, "ghidra_output"))
        ...
        if 'radare2' in modules_enabled:
            ...
            self.r2_ft.start()
            eprint("started r2 func thread on " +
                   self.path, color=C_UNDERLINE+C_YELLOW)
    ...
    def join_threads(self, timeout=30):
        ...
        for t in self.threads:
            if (t.is_alive()):
                eprint("waiting %fs for thread_%i:%s to terminate" %
                       (timeout, t.native_id, t.getName()), color=C_WARNING)
                # join will block
                t.join(timeout)
            ...
            if type(t) == cmdThread:
                ...
                eprint("cmd thread_%i:%s is done, stored output" %
                       (t.native_id, t.getName()), color=C_UNDERLINE)

            if type(t) == funcThread:
                eprint("function thread_%i:%s is done" %
                       (t.native_id, t.getName()), color=C_UNDERLINE)
        ...
        for mod in EXTERNAL_MODS:
            if isinstance(mod, NativeModule) and mod.threaded:
                ...
                if threads_done.get(modname) != None:
                    ...
                    eprint(f"ext threaded func {modname} output stored:", str(
                        self.thread_outputs[modname.lower()])[:70], color=C_UNDERLINE+C_GREEN)
    ...
    def get_info_output(self):
        ...
        try:
            ...
            eprint('[get_info_output] thread outputs',
                   self.thread_outputs.keys(), color=C_YELLOW+C_UNDERLINE)
            ...
            if self.thread_outputs.get('capa') != None:
                try:
                    ...
                    eprint("Error decoding capa json. Output: {}".format(
                        self.thread_outputs['capa'][:50]), color=C_RED)
            else:
                eprint("%s has no capa output" %
                       self.path, color=C_UNDERLINE+C_WARNING)

        except KeyError as e:
            eprint("KeyError: some output not ready / exist:",
                   e, color=C_UNDERLINE+C_WARNING)
        except AttributeError as e:
            eprint("AttributeError: some output not ready / exist:",
                   e, color=C_UNDERLINE+C_WARNING)
    ...
    def get_decomp_output(self):
        ...
        if self.thread_outputs.get('ghidra') != None:
            ...
            try:
                ...
                eprint(
                    f"get decompile output for {self.path} failed:", e, color=C_WARNING)
    ...
    def start_ghidra(self, output_path="ghidra_output"):
        ...
        output_c_file = os.path.join(
            OUTPUT_DIR, output_path, project_name + '.c')
        output_asm_file = os.path.join(
            OUTPUT_DIR, output_path, project_name + '.asm')
    ...
    def get_strings(self):
        ...
        eprint("[%f seconds] strings done on %s: %d bytes" % (
            time.time() - start, os.path.basename(self.path), len(strings)),  color=C_GREEN)
    ...
    def find_regex_strings(self):
        ...
        for s in regexes:
            ...
            eprint("[%f seconds] starting %s regex" %
                   (time.time() - start, s),  color=C_YELLOW)
            self.regex[s] = []
        ...
        eprint("[%f seconds] regex done on %s, found %d" % (
            time.time() - start, os.path.basename(self.path), found),  color=C_CYAN)
    ...
    def generate_hashes(self):
        ...
        eprint("[%f seconds] hashing done on %s" % (time.time() -
                                                   start, os.path.basename(self.path)),  color=C_GREEN)
    ...
    def analyze_graphs_r2(self):
        ...
        eprint("[%f seconds] r2 graphs output on %s: output sizes i:%d C:%d R:%d A:%d" % (time.time() - start, os.path.basename(self.path),
                                                                                          len(self.import_graph_json), len(self.global_call_graph_json), len(self.global_references_json), len(self.global_data_refs_json)), color=C_BLUE)
    ...
    def start_capa(self):
        ...
        self.capa_thread = cmdThread(
            "capa -j -r %s/capa-rules " % GLOBAL_START_PATH + self.path)
    ...
    def store_peinfo_json(self, pe, result):
        ...
        try:
            result['compile_time_repr'] = "%s UTC" % (
                time.asctime(time.gmtime(pe.FILE_HEADER.TimeDateStamp)))
        except ValueError:
            result['compile_time_repr'] = "Invalid Time {}: {}".format(
                pe.FILE_HEADER.TimeDateStamp, time.asctime(time.gmtime(pe.FILE_HEADER.TimeDateStamp)))
...
def run_binwalk(path):
    ...
    binwalk_runcmd = ["binwalk", "--dd=.*",
                      f"--directory={binwalkOutPath}", shlex.quote(os.path.realpath(path))]
    ...
    if not DEBUG:
        ...
        bw_ret = subprocess.call(
            binwalk_runcmd, stdout=sys.stdout, stderr=sys.stderr)

    eprint("[%f seconds] binwalk done on %s: returned with " %
           (time.time() - start, os.path.basename(path)), bw_ret, color=C_CYAN)

    # now store the result magic of files in json (when recursing, ignore the '0' file which is usually the original)
    try:
        os.chdir(os.path.join(binwalkOutPath, "_" +
                              os.path.basename(path) + ".extracted"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', action='store_true', help="list modules")
    parser.add_argument('-F', action='store_true', help="Run in fast mode, only enable fast modules")
    parser.add_argument('-T', help="-T <timeout>\ttotal timeout in seconds, default 30 per (binwalk) extracted PE file")
    parser.add_argument('-t', help="-t <timeout>\ttimeout per extracted PE file, default 30 (incompatible with -T)")
    parser.add_argument('-x', help="-x <modules>\trun all except these modules, comma separated")
    parser.add_argument('-m', help="-m <modules>\trun only these modules, comma separated (incompatible with -x)")
    ...
    if args.l:
        ...
        exit(0) 
    if args.T:
        total_timeout = float(args.T)
        eprint(f"total timeout set to {total_timeout}s", color=C_YELLOW)   
    if args.t:
        per_pe_timeout = float(args.t)
        eprint(
            f"per file timeout set to {per_pe_timeout}s", color=C_YELLOW)
    if args.m:
        modules_mode = 'include'
        include_list = args.m.split(',')
    if args.x:
        modules_mode = 'exclude'
        exclude_list = args.x.split(',')
    if args.d:
    ...
    eprint("external modules enabled: [", ([
           str(k.__class__.__name__) for k in EXTERNAL_MODS]), color=C_GREEN+C_BOLD)
    ...
    while threading.activeCount() > 1:
        ...
        eprint("\r[{}/{}s] {} threads left".format(check_thread_count,
                                                   timeout_s, threading.activeCount()), ','.join(names), color=C_BLUE)
        sleep_secs = 3
    ...
    for p in peinfo_all:
        ...
        eprint('before output_loc update: ', len(
            str(output_loc)), 'chars', color=C_LIME+C_UNDERLINE)
        output_loc.update(p.get_info_output())
        eprint('after output_loc update: ', len(
            str(output_loc)), 'chars', color=C_LIME+C_UNDERLINE)
    ...
    eprint(f"[✅ {time.time() - write_start}] done writing to all json files",
           color=C_LIME+C_BOLD)
    ...
    eprint(f"[+] done. analyzed %d samples, %d PE files, %d bytes of output written to {OUTPUT_DIR}" % (
        samples, pe_analyzed, outbytes), color=C_GREEN)
    eprint("[+] time elapsed: %f seconds, %fs / sample, %fs / PE file" %
           (time_elasped, sample_s, pe_s), color=C_GREEN)
    eprint("[+] exitting NOW")
    os._exit(0)
